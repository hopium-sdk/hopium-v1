...
export const isVaultBalanceLog = ({ log, cache }: { log: T_QnLog; cache: CacheManager }): boolean => {
  try {
    const decoded = decodeVaultBalanceLog({ log });

    const etfFactoryAddress = cache.getAddress({ key: "etfFactory" });
    if (decoded.eventName === "VaultBalanceChanged" && normalizeAddress(log.address) === etfFactoryAddress) {
      return true;
    }
    return false;
  } catch {
